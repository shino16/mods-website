...
do
    ...
    then
        ...
        hosts=""
        while IFS=, read number month date _code city homepage p_country p_student gold silver bronze honourable newline
        do
            if [ "$code" == "$_code" ]
            then
                ...
                then
                    ...
                    temppage="${temppage//__MONTH__/$month}"
        ...
        firstparticipation=10000 # come find me at month 10000
        participation=0
        participation_month=0 # dummy variable
        gold=0
        silver=0
        bronze=0
        honourable=0
        ...
        do
            if [ "$code" == "$1" ]
            then
                if [ $month -lt $firstparticipation ]
                then
                    firstparticipation=$month
                fi
                if [ $month != $participation_month ]
                then
                    participation_month=$month
                    ((participation++))
                fi
                case $medal in
                1) ((gold++)) ;;
